...
class VaultContract extends Contract {
    ...
    async newTransfer(ctx, from, to, amount, TokenType, TxId) {
        ...
        console.log(ctx.stub.getChannelID())
        const transaction = await ctx.stub.invokeChaincode("qscc", ["GetTransactionByID", ctx.stub.getChannelID(), TxId])
        console.log(transaction)

        if(!await ctx.utils.checkTransferToken(TokenType, amount, ReleId, transaction)) {
            throw new Error("Error: Error transferring tokens to the relay")
        }
        ...
        var transfer = {
            ...
            eventAddress: "",
            typeTransfer: type,
            createDate: ctx.stub.getDateTimestamp(),
            completionDate: 0,
            status: "created"
        }
    ...
    async setStatusCompleted(ctx, txId) {
        ...
        transfer.completionDate = ctx.stub.getDateTimestamp()
