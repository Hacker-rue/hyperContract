...
class Utils {
    ...
    async checkTransferToken(tokenType, amount, txId, releId) {
        var transaction = await queryTxFromLedger(await this.ctx.stub.invokeChaincode("qscc", ["GetTransactionByID", this.ctx.stub.getChannelID(), txId], this.sctx.stub.getChannelID()))
...
async function queryTxFromLedger(txn) {
    try {
        if(txn) {
